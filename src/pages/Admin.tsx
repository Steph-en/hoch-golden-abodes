...
const Admin = () => {
  ...
  const handleReactivateUser = (uid: string) => { if (!isSuperAdmin) return; rpc("reactivate_user", { _target_user_id: uid }, "User reactivated"); };
  const handleAssignRole  = (uid: string, role: string) => { if (!isSuperAdmin) return; rpc("assign_role",  { _target_user_id: uid, _role: role }, `Role "${role}" granted`); };
  const handleRevokeRole  = (uid: string, role: string) => { if (!isSuperAdmin) return; rpc("revoke_role",  { _target_user_id: uid, _role: role }, `Role "${role}" revoked`); };
  ...
  // ── Tab definitions ───────────────────────────────────────────────────────────
  const allTabs = [
    { id: "overview"       as AdminTabId, label: "Overview",       icon: BarChart3,     superOnly: true,  badge: 0 },
    ...
    { id: "enquiries"      as AdminTabId, label: "Enquiries",      icon: MessageSquare, superOnly: true,  badge: 0 },
    { id: "properties"     as AdminTabId, label: "Properties",     icon: Building2,     superOnly: false, badge: 0 },
    { id: "rooms"          as AdminTabId, label: "Rooms",          icon: BedDouble,     superOnly: false, badge: 0 },
    { id: "stays_bookings" as AdminTabId, label: "Stays Bookings", icon: CalendarCheck, superOnly: false, badge: 0 },
    ...
    { id: "activity"       as AdminTabId, label: "Activity",       icon: Activity,      superOnly: true,  badge: 0 },
  ];

  const visibleTabs = allTabs.filter(tab => canAccess(tab.id));

  // Map to SideNavItem format — use count only when > 0 so dot indicator shows
  const navItems: SideNavItem[] = visibleTabs.map(tab => ({
    id:    tab.id,
    label: tab.label,
    icon:  tab.icon,
  ...
  return (
    <div className="min-h-screen bg-background flex pt-16">
      ...
      <DashboardSidebar
        ...
        heading="Admin Panel"
        subheading={isSuperAdmin ? "Super Admin · HOCH Group" : "Admin · HOCH Group"}
      />
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10 py-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <div className="flex flex-wrap items-center gap-3 mb-3">
              ...
              <div>
                <h1 className="font-display text-2xl md:text-3xl font-semibold text-foreground flex items-center gap-2">
                  {visibleTabs.find(t => t.id === activeTab)?.label ?? "Admin"}
                  {isSuperAdmin && (
                                          ...
                                          <AlertDialogContent>
                                            ...
                                            <AlertDialogFooter>
                                              ...
                                              <AlertDialogAction className="bg-destructive hover:bg-destructive/90"
                                                onClick={() => handlePermDeleteUser(u.user_id, u.email)}>Delete</AlertDialogAction>
                                            </AlertDialogFooter>
                                          </AlertDialogContent>
                                        </AlertDialog>
                                      </>
